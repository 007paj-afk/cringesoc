-- Holy ai Coded Wtffff 
-- Ts is not good :skull:
...
for i,lc2 in pairs(game.Players.LocalPlayer.
...
local x = {
	game:GetService("ReplicatedStorage").Events.AntiCheatRemote,
	game:GetService("ReplicatedStorage").Events.SecretEvent,
	game:GetService("ReplicatedStorage").Events.SusRemote,
}
...
-- Update message
message.Text = "man yall game is shit but here just made it "

-- Wait before fading out
task.wait(2)

-- Tween out frame and text
local fadeOutTime = 1
...
-- Wait for fade-out to finish, then clean up
...
local Window = WindUI:CreateWindow({
    Title = "Alantic.dev | Cringe Soccer",
    ...
    OpenButton = {
        Title = "Open Twistzz Script", -- can be changed
...
-- REMOTES
local KickRemote =
    workspace:WaitForChild("FE", 9e9)
        :WaitForChild("Actions", 9e9)
        :WaitForChild("Kick", 9e9)

local TPS =
    workspace:WaitForChild("TPSSystem", 9e9)
        :WaitForChild("TPS", 9e9)

-- STATE
local powershootEnabled = false
local powerValue = 180 -- default

-- KICK FUNCTION
local function doPowerShoot()
    if not powershootEnabled then return end

    local args = {
        [1] = TPS,
        [2] = powerValue, -- slider controlled
        [3] = Vector3.new(900000, 850, 900000),
        [4] = false,
        [5] = false,
        [6] = 1
    }
...
-- TOGGLE
PS:Toggle({
    Title = "Enable / Disable Powershoot (PRESS H TO POWERSHOOT)",
    Callback = function(Value)
        powershootEnabled = Value
    end
})
